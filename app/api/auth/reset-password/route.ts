  ...
  try {
    ...
    if (!user.phone) {
      return NextResponse.json(
        {
          error:
            "No phone number on file. Please contact your administrator to reset your password.",
        },
        { status: 400 }
      )
    }
    ...
    if (!isSmsConfigured) {
      return NextResponse.json(
        {
          error:
            "Password reset is not available. SMS service is not configured. Contact your administrator.",
        },
        { status: 503 }
      )
    }
    ...
    try {
      ...
      logger.error("Failed to set password after SMS sent", {
        userId: user.id,
