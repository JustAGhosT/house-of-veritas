...
export function BarcodeLabelGenerator({ items, onClose }: BarcodeLabelGeneratorProps) {
  ...
  const getLabelDimensions = () => {
    switch (config.labelSize) {
      case "small": return { width: 180, height: 80, fontSize: 10 }
      case "large": return { width: 300, height: 150, fontSize: 14 }
      default: return { width: 240, height: 100, fontSize: 12 }
    }
  }
  ...
  const printLabels = () => {
    ...
    const labelsHtml = selectedItems.map(id => {
  ...
  const downloadSVG = () => {
    ...
    itemsToExport.forEach((item, index) => {
      const barcode = item.barcode || `HV-${item.id}`
      const svg = config.barcodeType === 'qrcode'
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Select value={config.labelSize} onValueChange={(v: any) => setConfig(p => ({ ...p, labelSize: v }))}>
            ...
            <div className="space-y-2">
              ...
              <Select value={config.barcodeType} onValueChange={(v: any) => setConfig(p => ({ ...p, barcodeType: v }))}>
                <SelectTrigger className="bg-white/5 border-white/10">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="rounded border-white/20"
              />
              <span className="text-white text-sm">Show Barcode</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="rounded border-white/20"
              />
              <span className="text-white text-sm">Show Category</span>
            </label>
          </div>
        </CardContent>
      </Card>
            ...
            {items.map((item) => (
              <label
                ...
                className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors ${
                  selectedItems.includes(item.id) ? 'bg-blue-500/20 border-blue-500' : 'bg-white/5 hover:bg-white/10'
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-white/40 text-xs font-mono">{item.barcode || `HV-${item.id}`}</p>
                </div>
              </label>
            ))}
          </div>
      ...
      <Card className="bg-white/5 border-white/10">
        ...
        <CardContent>
          <div ref={printRef} className="flex flex-wrap gap-2 justify-center p-4 bg-white rounded-lg">
            {selectedItems.slice(0, 3).map(id => {
              ...
              return (
                <div
                  ...
                  className="border border-gray-300 p-2 inline-block text-black"
                >
                  {config.showName && (
                    ...
                    <div className="mt-1">
                      <div
                        dangerouslySetInnerHTML={{
                          __html: config.barcodeType === 'qrcode'
                            ? generateQRCodeSVG(barcode, dims.width - 16)
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button variant="outline" onClick={downloadSVG} disabled={selectedItems.length === 0} className="border-white/10">
          <Download className="h-4 w-4 mr-2" />
          Download SVGs
        </Button>
        <Button onClick={printLabels} disabled={selectedItems.length === 0} className="bg-blue-600 hover:bg-blue-700">
...
export function LabelPrintDialog({ items, open, onOpenChange }: LabelPrintDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[#0d0d12] border-white/10 text-white max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
